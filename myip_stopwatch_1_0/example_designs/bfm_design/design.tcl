proc create_ipi_design { offsetfile design_name } {
	...
	set myip_stopwatch_0 [ create_bd_cell -type ip -vlnv user.org:user:myip_stopwatch:1.0 myip_stopwatch_0]
...
set_property -name {xsim.simulate.runtime} -value {100ms} -objects [get_filesets sim_1]
	...
	set offset_file "${bd_path}/myip_stopwatch_tb_include.svh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef myip_stopwatch_tb_include_vh_"
	puts $fp "`define myip_stopwatch_tb_include_vh_\n"
...
set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores user.org:user:myip_stopwatch:1.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/myip_stopwatch_tb.sv
...
for { set i 1 } { 1 } { incr i } {
	set design_name "myip_stopwatch_bfm_${i}"
...
remove_files -quiet -fileset sim_1 myip_stopwatch_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top myip_stopwatch_tb [get_filesets sim_1]
